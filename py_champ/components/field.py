...
class Field_aquacrop(mesa.Agent):
    """
    {Michelle: we need to update this description focusing on the coupling with AquaCrop. Also clean all the functions from parameters that we are not using anymore (e.g. water yield curves)}
    This module is a field simulator.
    ...
    **kwargs
    ...
    def load_settings(self, settings: dict):
        ...
        ----------
        settings : dict
            A dictionary containing settings for the field, including field area, water yield curves for crops, and technological coefficients.
        """
        crop_options = self.model.crop_options
        self.field_area = settings["field_area"]
        self.water_yield_curves = settings["water_yield_curves"]
    ...
    def update_crops(self, i_crop):
        ...
        ----------
        i_crop : 3d array
            Indicator array representing the chosen crops for the next year for each area split. The dimension of the array should be (n_s, n_c, 1).
    ...
    def write_to_csv(self, filename="default.csv"):
        ...
        filename : str, optional
            Name of the CSV file to write to, default is 'default.csv'.
        """
        with open(filename, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['max_irrseason', 'crop_name', 'irrig_method'])
    ...
    def step(self, irr_depth, i_crop, i_te, prec_aw: dict) -> tuple:
        ...
        # Update crops and irrigation technology
        self.update_crops(i_crop)
        self.update_irr_tech(i_te)

        # Prepare data for CSV output
        max_irrseason = irr_depth.flatten().tolist()
        crop_name = i_crop.flatten().tolist()
        irrig_method = []

        for method in i_te:
            if method == "irrigated":
                irrig_method.append(1)
            elif method == "rainfed":
                irrig_method.append(0)
            else:
                #raise ValueError(f"Unknown irrigation method: {method}")
                irrig_method.append(3)

        # Append data to csv_data list
        self.csv_data.append([max_irrseason, crop_name, irrig_method])

        # Calculate and return yield, yield rate, and irrigation volume
        yield_result = self.calculate_yield(irr_depth, i_crop, i_te, prec_aw)

        # After calculating yield, write data to CSV
        self.write_to_csv("/Users/michellenguyen/Downloads/default.csv")
